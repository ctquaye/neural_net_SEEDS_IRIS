...
for epoch in range (int(num_epochs)):
        ...
        if (int(print_internals_flag) == 1):
            ...
            for row in range(len(Matrix)):
                for column in range(len(Matrix[row])):
                    ...
                    print(Matrix[row][column], end=' ')
                print()
    if (((int(epoch)) % 10 != 0 and (int(epoch)) >= 10) and int(epoch) != hi_train_rng):
        for train in range(int(hi_train_rng) - int(low_train_rng)):
            ...
            for ct in range(output_out_len):  # var is still equal to layer number of output layer
                for it in range(len(Matrix[len(Matrix) - 1][2:-1])):  # range of weights
                    to_sig = to_sig + (hidden_chunk[it] * Matrix[start_of_outlayer+ct][it+offset])
            ...
            for fl in range(len(Matrix[start_of_outlayer][2:-1])):
                for tl in range (len(Matrix[start_of_outlayer:])):
                   to_hid_delta = to_hid_delta +(np.multiply(Output_delta_arr[tl],Matrix[tl+start_of_outlayer][fl+offset]))
    ...
    if (((int(epoch)) % 10 == 0 and (int(epoch)) >= 10) or int(epoch)==hi_train_rng):
        correctTest = 0
        ...
        print(epoch, ": ", correctTest, "/", test_rng, round(correctTest / test_rng, 4))
        #print(epoch, ": ", correctTrain, "/", train_rng, round(correctTrain / train_rng, 4))
